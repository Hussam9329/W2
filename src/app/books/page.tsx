...
const TYPE_CONFIG: Record<TabType | 'movie' | 'anime' | 'series', { icon: typeof Film; label: string; plural: string; color: string; bgColor: string }> = {
  all: { icon: Bookmark, label: 'الكل', plural: 'جميع الكتب', color: 'from-blue-500 to-sky-500', bgColor: 'bg-blue-500/10' },
  book: { icon: BookOpen, label: 'كتاب', plural: 'كتب', color: 'from-blue-300 to-blue-500', bgColor: 'bg-[#f0d77a]/10' },
  movie: { icon: Film, label: 'فيلم', plural: 'أفلام', color: 'from-blue-500 to-sky-500', bgColor: 'bg-blue-500/10' },
  anime: { icon: Sparkles, label: 'أنمي', plural: 'أنميات', color: 'from-blue-400 to-blue-600', bgColor: 'bg-blue-500/10' },
  series: { icon: Tv, label: 'مسلسل', plural: 'مسلسلات', color: 'from-blue-300 to-blue-500', bgColor: 'bg-blue-400/10' }
}
...
export default function WatchListPage() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white">
      <div className="fixed inset-0 pointer-events-none"><div className="absolute top-0 right-0 w-[800px] h-[800px] bg-gradient-to-bl from-blue-500/5 to-transparent rounded-full blur-3xl" /><div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-gradient-to-tr from-blue-600/5 to-transparent rounded-full blur-3xl" /></div>
      <div className="relative z-10 max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8" dir="rtl">
        <header className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3 sm:gap-5">
            ...
            <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-500 to-sky-500 flex items-center justify-center shadow-lg"><Bookmark className="w-5 h-5 sm:w-7 sm:h-7 text-white" /></div><div><h1 className="text-lg sm:text-2xl font-bold">أريد قرائته</h1><div className="flex items-center gap-2"><p className="text-neutral-500 text-sm">قائمة القراءة</p><span className={`text-xs flex items-center gap-1 ${syncStatus === 'synced' ? 'text-green-500' : syncStatus === 'syncing' ? 'text-yellow-500' : 'text-red-500'}`}>{syncStatus === 'synced' && <Cloud className="w-3 h-3" />}{syncStatus === 'syncing' && <Loader2 className="w-3 h-3 animate-spin" />}{syncStatus === 'error' && <CloudOff className="w-3 h-3" />}{syncStatus === 'synced' ? 'متزامن' : syncStatus === 'syncing' ? 'مزامنة...' : 'خطأ'}</span></div></div></div>
          <div className="flex items-center gap-1 sm:gap-2"><Button onClick={() => setShowStats(!showStats)} variant="ghost" size="icon" className="text-neutral-400 hover:text-white"><BarChart3 className="w-5 h-5" /></Button><Popover><PopoverTrigger asChild><Button variant="ghost" size="icon" className="text-neutral-400 hover:text-white"><Settings className="w-5 h-5" /></Button></PopoverTrigger><PopoverContent className="w-48 bg-[#1a1a1a] border-[#2a2a2a]"><div className="space-y-2"><Button onClick={exportData} variant="ghost" className="w-full justify-start gap-2 text-white hover:bg-[#2a2a2a]"><Download className="w-4 h-4" />تصدير</Button><Button onClick={() => importInputRef.current?.click()} variant="ghost" className="w-full justify-start gap-2 text-white hover:bg-[#2a2a2a]"><UploadIcon className="w-4 h-4" />استيراد</Button><input ref={importInputRef} type="file" accept=".json" className="hidden" onChange={importData} /></div></PopoverContent></Popover><Button onClick={() => { resetForm(); setShowAddDialog(true) }} className="bg-gradient-to-br from-blue-500 to-sky-500 text-white font-bold gap-2"><Plus className="w-4 h-4" /><span className="hidden sm:inline">إضافة</span></Button></div>
        </header>

        {showStats && <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-xl p-6 mb-6"><h3 className="font-bold mb-4 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-blue-500" />إحصائيات {TYPE_CONFIG[activeTab].plural}</h3><div className="grid grid-cols-2 md:grid-cols-4 gap-4"><div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]"><p className="text-2xl font-bold text-blue-500">{stats.total}</p><p className="text-sm text-neutral-400">إجمالي</p></div><div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]"><p className="text-2xl font-bold text-blue-300">{stats.watched}</p><p className="text-sm text-neutral-400">تمت القراءة</p></div><div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]"><p className="text-2xl font-bold text-blue-400">{stats.favorite}</p><p className="text-sm text-neutral-400">مفضلة</p></div><div className="bg-[#1a1a1a] rounded-lg p-4 border border-[#2a2a2a]"><p className="text-2xl font-bold text-blue-500">{stats.avgRating.toFixed(1)}</p><p className="text-sm text-neutral-400">متوسط التقييم</p></div></div></div>}

        <div className="flex gap-2 mb-4 sm:mb-6 overflow-x-auto mobile-tabs-scroll pb-1 sm:pb-0">{(['all', 'book'] as TabType[]).map((type) => { const c = TYPE_CONFIG[type]; const I = c.icon; const active = activeTab === type; return <button key={type} onClick={() => setActiveTab(type)} className={`flex-shrink-0 min-w-[90px] sm:min-w-0 rounded-xl p-3 transition-all ${active ? 'bg-gradient-to-br ' + c.color + ' text-white shadow-lg' : 'bg-[#1a1a1a] text-neutral-400 hover:bg-[#2a2a2a] border border-[#2a2a2a]/50'}`}><div className="flex items-center gap-2"><I className="w-5 h-5" /><div className="text-right"><p className="font-bold text-sm">{c.plural}</p><p className={`text-xs ${active ? 'opacity-80' : 'text-neutral-500'}`}>{tabStats[type].watched}/{tabStats[type].total}</p></div></div></button> })}</div>
        ...
        <div className="bg-[#0f0f0f] border border-[#2a2a2a] rounded-xl p-3 sm:p-4 mb-6">
          <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto mobile-toolbar pb-1 sm:pb-0">
            <div className="flex-1 min-w-0 relative"><Search className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-500" /><Input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="بحث..." className="bg-[#1a1a1a] border-[#2a2a2a] focus:border-blue-500 pr-9 h-10" /></div>
            ...
            <div className="flex bg-[#1a1a1a] rounded-lg p-1"><Button variant="ghost" size="icon" onClick={() => setViewMode('grid')} className={`h-8 w-8 ${viewMode === 'grid' ? 'bg-gradient-to-br from-blue-500 to-sky-500 text-white' : 'text-neutral-400'}`}><Grid3X3 className="w-4 h-4" /></Button><Button variant="ghost" size="icon" onClick={() => setViewMode('list')} className={`h-8 w-8 ${viewMode === 'list' ? 'bg-gradient-to-br from-blue-500 to-sky-500 text-white' : 'text-neutral-400'}`}><List className="w-4 h-4" /></Button></div>
            <Button variant="outline" onClick={() => setShowFilters(!showFilters)} className={`gap-2 h-10 ${showFilters ? 'border-blue-500 text-blue-500' : 'border-[#2a2a2a] text-neutral-400'}`}><Filter className="w-4 h-4" /><span className="hidden sm:inline">فلاتر</span></Button>
          </div>
