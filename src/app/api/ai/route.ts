...
export async function POST(request: NextRequest) {
  try {
    ...
    const { action, context, ideaTitle, ideaContent, ideaCategory, conversation, messages } = body

    console.log('AI API received action:', action)
...
3. اقتراح لتعزيز التنفيذ`
      ...
      case 'chat':
        systemPrompt += '\n\nالمستخدم يتحدث معك عن أفكاره. أجب بذكاء وساعد في تطوير الأفكار. كن ودوداً ومحفزاً.'
