...
export default function IdeasPage() {
  ...
  useEffect(() => {
    try {
      const storedIdeas = localStorage.getItem('riham_ideas')
      if (storedIdeas) setIdeas(JSON.parse(storedIdeas))
      const storedTasks = localStorage.getItem('riham_tasks')
  ...
  useEffect(() => {
    try {
      localStorage.setItem('riham_ideas', JSON.stringify(ideas))
    } catch {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('riham_tasks', JSON.stringify(tasks))
    } catch {
      // ignore storage errors
    }
  }, [tasks])
  ...
  const exportData = useCallback(() => {
    ...
    a.href = url
    a.download = `riham_backup_${new Date().toISOString().split('T')[0]}.json`
    a.click()
